...
description: Combined single-session orchestrator for the entire Unified suite (Brief, Mail, Invest) — the recommended design, replacing Orchestrator.md (v1) for scheduled/unattended use. Instead of running each product fully to completion in strict sequence (the shape that ran out of runtime budget on 2026-07-21), this version runs Preparation for all three products up front, then fans every active collector out in ONE parallel batch across all three products — adapting the flat parallel-collection shape already proven reliable in Daily-Sprint15 — then aggregates, narrates, and logs/caches per product. Validated live end-to-end on 2026-07-21 (all 6 collectors launched as real concurrent sub-agents, completed cleanly, no failures). Trigger: "Start Unified v2", or invoked directly by a scheduled routine with no prior conversation context.
...
system: |-
  ...
  Validated live end-to-end on 2026-07-21: all 6 collectors (Investment-
  Signals, Email-Summary, Newsletter-Digest, News-Collector, Field-Monitor,
  Daily-Brief) were launched as genuinely concurrent background sub-agents
  and all completed successfully with no failures, no runtime/budget issues,
  and no manual intervention. This is now the RECOMMENDED design for
  scheduled/unattended execution of the full suite, replacing both v1's
  combined Orchestrator.md and the three-separate-triggers workaround.

  This only works if Step 3's collectors are actually launched concurrently
  — genuinely parallel tool/agent invocations that run at the same time —
  not sequentially while merely being described as "parallel." A cold
  session executing this file must use real concurrent execution (e.g.
  background sub-agents, one per collector, launched in a single batch) for
  Step 3. Running them one after another while following this file's prose
  would silently reintroduce the exact sequential-chaining problem this
