# Sprint 8 – AI Agent Execution Prompts

Based on Sprint 7. Key change: all report outputs are saved to **Reports15 (Google Drive)** instead of Reports10 (Notion under Admin). Reports15 folder ID: `1aa0mOKOM_n_z2VkpU8r-Q7HgoVfdD-p_` (https://drive.google.com/drive/folders/1aa0mOKOM_n_z2VkpU8r-Q7HgoVfdD-p_). Execution Log and AI Agents Memory remain in Notion under Release folder sharp → Execution documents — unchanged.
...
```
Reports15 (Google Drive, folder ID: 1aa0mOKOM_n_z2VkpU8r-Q7HgoVfdD-p_)
...
**Step 4 — Log**
Add one entry to **Execution Log** (under **Release folder sharp → Execution documents**) for today (title: Month DD, YYYY; short bullet summary). Overwrite if today's entry already exists.
...
Logic:
- List all folders inside Reports15 (Google Drive, folder ID: `1aa0mOKOM_n_z2VkpU8r-Q7HgoVfdD-p_`).
- Filter to folders whose name starts with "📁 Collection –".
- Sort by date (newest first).
- Keep the 5 most recent Collection folders.
- Move Collection folders 6 and beyond to Trash in Google Drive.

⚠️ Manual step: permanently delete trashed items from Google Drive Trash by hand — Claude cannot permanently delete files.
...
**Orchestrator:** Run 3 agents in strict order using **AI Agents Memory** (under **Release folder sharp → Execution documents**) as shared storage. Report counts after each step.
...
For each new signal, create a short summary Google Doc **inside the Collection folder** (parent = Collection Drive folder ID) titled "Investment Signal - [short description] ([Month DD, YYYY])". Include: ticker, recommendation, confidence, reasoning, link to Investment Signals database.
