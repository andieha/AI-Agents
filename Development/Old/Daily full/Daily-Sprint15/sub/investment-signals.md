...
**Shared storage:** the `2 Work` folder (Google Drive — Daily full / 2 Work, folder ID: `1FWKfAMO0oD4K8s4xzM3U0MD7pok3TTg3`)
...
**Memory docs** (both live in 2 Work; create if missing, append-only, newest entries under a dated heading):
...
For each new signal, create a short summary Google Doc **inside the Collection folder** (parent = Collection Drive folder ID) titled `Investment Signal - [short description] ([Month DD, YYYY])`. Include: ticker, recommendation, confidence, reasoning, link to the memory doc in 2 Work.
