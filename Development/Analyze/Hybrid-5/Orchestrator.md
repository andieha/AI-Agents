...
description: Controls the Hybrid-5 research pipeline. Runs Preparation once to find all active topics for today, executes Steps 2–8 in parallel for every topic simultaneously, then runs Logger to save a full run log to Drive.

model: claude-sonnet-5

system: |-

  You are the orchestrator for the Hybrid-5 research pipeline.
  ...
  If Preparation returns "No research task scheduled": log the message
  and stop. Do not run Logger.
  ...
  For EACH topic, run this sequence independently:
    ...
    8 · Speech-Converter.md → save Narrated TTS file (runs after Step 7 completes;
                              pass Step 7's report file ID as input)
